...
const RoomOwnerBookingHistoryList: React.FC<IRoomInfoProps> = () => {
  ...
  const fetchBookingData = async () => {
    const res = await fetch(`${REACT_APP_API_SERVER}/booking/booking-record/${id}`, {
      method: "GET",
      headers: {
    ...
    if (bookingList.length > 0) {
      ...
      for (let i = 0; i < bookingList.length; i++) {
        if (bookingList[i].time_slots.length === 1) {

        } else if (bookingList[i].time_slots.length > 1) {
          bookingList[i].time_slots = bookingList[i].time_slots.reduce((prev: any, current: any) => {
            return prev + current
          })
        }
      }
      ...
      <div className={classes.root}>
        ...
        <TabPanel value={value} index={0}>
          ...
          {completedRequest.length > 0 && completedRequest.map((data: any, idx: any) => (
            <NavLink className={styles.box_href} to={`/room-owner/booking-history-detail/${data.id}`}>
                  ...
                  <div>
                    <div className={styles.user_info_section}>
                      <div className={styles.user_info}>
                        <div className={styles.request_info}>
                          Visitor: <span className={styles.span_info}>{data.username}</span>
                        </div>
                        <div className={styles.request_info}>
                          Number of visitors: <span className={styles.span_info}>{data.head_count}</span>
                        </div>
                      </div>
                      <div className={styles.user_pic}>
                        <img src={data.profile_picture.slice(0, 5) === "https" ? data.profile_picture : REACT_APP_API_SERVER + "/" + data.profile_picture} alt="profile-pic" />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className={styles.each_grid_info}>
                  ...
                  <div>
                    <div className={styles.user_info_section}>
                      <div className={styles.user_info}>
                        <div className={styles.request_info}>
                          Username: <span className={styles.span_info}>{data.username}</span>
                        </div>
                        <div className={styles.request_info}>
        ...
        <TabPanel value={value} index={2}>
          {pendingRequest.length === 0 && <div>
            ...
            <br></br>
            <br></br>
            <span className="no-photos-at-moment">Currently No Pending Time-Slot Request.</span>
          </div>}
          {pendingRequest.length > 0 && pendingRequest.map((data: any, idx: any) => (
            <NavLink className={styles.box_href} to={`/room-request/${data.id}`}>
              <Paper className={styles.card_piece} elevation={3}>
                <div className={styles.each_grid_info}>
                  <div className={styles.request_basic_info}>
                    <div className={styles.request_info}>
        ...
        <TabPanel value={value} index={3}>
          {rejectedRequest.length === 0 && <div>
            ...
            <br></br>
            <br></br>
            <span className="no-photos-at-moment">Currently No Rejected Time-Slot Request.</span>
          </div>}
...
export default RoomOwnerBookingHistoryList;